...
def activate_account(request, uid, token):
    ...
    DOMAIN = getattr(settings, "DOMAIN") or "server.manarat-elsakan.com"
    ACTIVATE_ACCOUNT_URL = getattr(settings, "ACTIVATE_ACCOUNT_URL") or "accounts/users/activation/"
    return render(
        request,
...
def reset_password_confirm(request, uid, token):
    PROTOCOL = getattr(settings, "PROTOCOL") or "https"
    DOMAIN = getattr(settings, "DOMAIN") or "server.manarat-elsakan.com"
    RESET_PASS_CONFIRM_URL = getattr(settings, "RESET_PASS_CONFIRM_URL") or "accounts/users/reset_password_confirm/"
    return render(
        request,
